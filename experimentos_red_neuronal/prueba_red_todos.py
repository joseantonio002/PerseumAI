...
from network_utils import transform, CNN_all
...
def classify(image_tensor, model):
    predicted_final = -1
    confidence_final = -1
    model.eval() 
    with torch.no_grad():
...
def MIfindCurrentPatterns_automatico(company_name, models):
  ...
  found_patterns = deleteRepeatedPatterns(found_patterns)
...
if __name__ == '__main__':
    ...
    for empresa in companies:
        ...
        #MIfindCurrentPatterns_automatico(empresa, model)
        trainHistoricDatabase(empresa, datetime(2021, 1, 2), datetime(2024, 1, 2), model)

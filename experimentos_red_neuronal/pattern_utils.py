...
def enhanceDataframeWithNeuralNetwork(distance_found, sliced_vector, window_divisions, model_pattern):
    ...
    for number_of_parts in window_divisions:
        ...
        for i in range(number_of_parts):
            ...
            image = convertir_patron_a_imagen(normalized_split_vector)
            image_tensor = transform(image)
            predicted, confidence = classify(image_tensor, model_pattern)
            if predicted >= 0 and confidence >= minimum_distance:
                minimum_distance = confidence
...
def deleteRepeatedPatterns(patterns_found):
    ...
    for index, pattern in enumerate(patterns_found):
        for index2, pattern2 in enumerate(patterns_found):
            ...
            # si es el mismo tipo de patrón, misma empresa y la fecha del patrón 1 
            if (pattern.pattern_type == pattern2.pattern_type and 
                ...
                starting_date_pattern <= end_date_pattern2 <= end_date_pattern and
                index != index2):
                patterns_found.pop(index2)

...
class MenuWindow:
    ...
    def runProgram(self):
        ...
        if self.companies == None:
            ...
            return
        if int(self.window_entry.get()) < MININUM_WINDOW_SIZE:
        ...
        patterns_dictionary = pattern_utils.loadPatterns(NUMBER_PATTERNS_TO_LOAD, selected_types_set)
        historic_results = []
        current_results = []
        ...
        for company in self.companies:
            ...
            start = time.time()
            historic_results = historic_results + mn.trainHistoricDatabaseAutomatic(company, patterns_dictionary, self.InitialDate.get_date(), self.EndingDate.get_date(), int(self.window_entry.get()))
            end = time.time()
        ...
        '''
        #-----Guardar patrones históricos como imágenes en la carpeta collected_data/tipo_patron-----
        ...
        for pattern in historic_results:
          ...
          ax.plot(pattern.dataframe_segment.iloc[:, 0])
          ax.axis('off')
          ...
          filename = f'{pattern.company_name}{pattern.starting_date[5:7]}{pattern.starting_date[8:10]}{pattern.ending_date[5:7]}{pattern.ending_date[8:10]}'
          final_output_dir = os.path.join(output_dir, pattern.pattern_type)

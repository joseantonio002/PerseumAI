...
from pattern_utils import deleteRepeatedPatterns, count_source, calculate_coincidences
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
  ...
  def runProgram(self, button, selected_types):
    ...
    historic_results = []
    current_results = []
    divisions = self.get_divisions(self.comboBox.currentText())
    ...
    for company in self.companies:
      ...
      if button == self.historicpatternsbtn_both:
        historic_results = historic_results + self.trainHistoric(company, patterns_dictionary, self.models, self.initialdate.date().toPyDate(), self.enddate.date().toPyDate(), divisions, search_mode)
      elif button == self.currentpatternsbtn_both:
        current_results = current_results + self.findCurrent(company, patterns_dictionary, self.models, divisions, search_mode)
      self.progressBar.setValue(company_index)
    count_source(historic_results)
    # patrones encontrados con dtw sin repetir, patrones encontrados con nn sin repetir, patrones repetidos, número de coincidencias, patrones históricos (eliminando repetidos)
    historic_results, non_repeated_patterns, repeated_patterns, coincidences = calculate_coincidences(historic_results)
    #guardar patrones en sus respectivos directorios-------------------
    for pattern in non_repeated_patterns:
      if pattern.source == 'DTW':
        self.savePatternAsImage(pattern, f'./imagenes_resultados/DTW/{pattern.company_name}_{pattern.pattern_type}_{pattern.starting_date[:10]}_{pattern.ending_date[:10]}_{pattern.source}.png')
      else:
        self.savePatternAsImage(pattern, f'./imagenes_resultados/NN/{pattern.company_name}_{pattern.pattern_type}_{pattern.starting_date[:10]}_{pattern.ending_date[:10]}_{pattern.source}.png')
    for pattern in repeated_patterns:
      self.savePatternAsImage(pattern, f'./imagenes_resultados/repetidos/{pattern.company_name}_{pattern.pattern_type}_{pattern.source}_{pattern.starting_date[:10]}_{pattern.ending_date[:10]}.png')
    for pattern in historic_results:
      self.savePatternAsImage(pattern, f'./imagenes_resultados/resultados_finales/{pattern.company_name}_{pattern.pattern_type}_{pattern.source}_{pattern.starting_date[:10]}_{pattern.ending_date[:10]}.png')
    #-------------------------------------------------------------------
  ...
  def trainHistoric(self, company, patterns_dictionary, models, initial_date, end_date, divisions, search_mode):
    historic_results = None
    if search_mode == "DTW and Neural Network":
      historic_results = mn.trainHistoricDatabaseNetwork(company, patterns_dictionary, initial_date, end_date, models, divisions)
      historic_results = historic_results + mn.trainHistoricDatabaseAutomatic(company, patterns_dictionary, initial_date, end_date, divisions)
  ...
  def savePatternAsImage(self, pattern, image_route):
      ...
      fig.savefig(image_route)
  
  def findCurrent(self, company, patterns_dictionary, models, divisions, search_mode):
    current_results = None
    if search_mode == "DTW and Neural Network":
      current_results = mn.findCurrentPatternsAutomatic(company, patterns_dictionary, divisions)
      current_results = current_results + mn.findCurrentPatternsNetwork(company, models, divisions)
    elif search_mode == "DTW":
      current_results = mn.findCurrentPatternsAutomatic(company, patterns_dictionary, divisions)
    elif search_mode == "Neural Network":
      current_results = mn.findCurrentPatternsNetwork(company, models, divisions)
  ...
  def get_divisions(self, text):
    """
    Returns the list of divisions based on the selected search type.
        
    Args:
      text (str): The selected search type
        
    Returns:
      list[int]: The list of divisions for the search type
    """
